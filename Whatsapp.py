...
print("User data will be saved in: {}".format(os.path.join(sys.path[0], "UserData")))
...
class Whatsapp:
    ...
    def login(self):
        ...
        if not self.__isLogin():
            ...
            print(
                "After 3 sec - Screenshot of QR code will be saved in: {}".format(
                    os.path.join(sys.path[0], "QRCode.png")
                )
            )
            time.sleep(3)
            self.browser.save_screenshot(os.path.join(sys.path[0], "QRCode.png"))
    ...
    def __wait(self, cName, timeout=60):
        print(
            "Waiting for element: {}".format(cName),
            " To load, timeout: {}".format(timeout),
            " seconds remaining",
        )
        wait = WebDriverWait(self.browser, timeout)
        try:
            element = wait.until(EC.presence_of_element_located((By.CLASS_NAME, cName)))
        except:
    ...
    def __saveToCSV(self, data, name):
        ...
        with open(fileName, "w", encoding="utf-8", newline="") as file:
            ...
            writer.writerow(
                ("Date", "Sender", "Message", "Replied To", "Replied Message")
            )
    ...
    def __search(self, query):
        self.browser.find_element(By.CLASS_NAME, "copyable-text").send_keys(query)
        self.browser.save_screenshot(os.path.join(sys.path[0], "Searched !.png"))
        time.sleep(0.25)
    ...
    def getMessages(
        ...
        scroll=None,
        manualSync=False,
        element="_1AOLJ._1jHIY",
    ):
        self.__openChat(chatName)

        self.__wait(element)

        messages_elements = self.browser.find_elements(By.CLASS_NAME, element)
        if len(messages_elements) > 0:
            messages_elements[0].click()
        else:
        ...
        for message_element in messages_elements:
            try:
                msg = self.__parseMessage(message_element)
                fetchedMessages.append(msg)
            except:
                print("fetch error")
                continue  # Skip this element if it becomes stale during iteration
    ...
    def replyTo(self, element, msg):
        ...
        while True and totalretries > 0:
            try:
                ...
                ActionChains(self.browser).move_to_element(
                    element.find_elements(By.CLASS_NAME, "l7jjieqr.fewfhwl7")[0]
                ).click().perform()
    ...
    def __parseMessage(self, message):
        try:
            msg = message.find_element(By.CLASS_NAME, "_21Ahp").text
        except:
            msg = "MEDIA"
        ...
        try:  # For all chat
            ...
            repliedMsg = message.find_element(
                By.CLASS_NAME, "quoted-mention._11JPr"
            ).text
            # If didnt crash, means there is reply
            # Check for reply to for Other
            try:
                repliedTo = message.find_elements(By.CLASS_NAME, "_3FuDI._11JPr")[
                    -1
                ].text  # Any Reply to Other Person
        ...
        try:
            date = message.find_elements(By.CLASS_NAME, "l7jjieqr.fewfhwl7")[-1].text
        except:
            date = "NONE"
        ...
        try:
            ...
            msgSender = message.find_element(
                By.CLASS_NAME, "_3IzYj._6rIWC.p357zi0d"
            ).text
            ...
            if "message-out" in message.get_attribute("class"):
                ...
                try:
                    ...
                    )[0].get_attribute("data-pre-plain-text")
                    msgSender = data_plain_t[
                        data_plain_t.find("] ") + 2 : -2
    ...
    def reply_to_message(self, chat_name, message_text, replied_message_text):
        self.__openChat(chat_name)

        self.__wait("_1AOLJ._1jHIY")  # Wait for the chat messages to load

        # Find the message to which you want to reply
        messages_elements = self.browser.find_elements(By.CLASS_NAME, "_1AOLJ._1jHIY")
        for message_element in messages_elements:
            if message_element.text == replied_message_text:
                self.replyTo(message_element, message_text)
